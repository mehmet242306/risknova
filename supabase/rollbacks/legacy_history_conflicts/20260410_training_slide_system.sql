...
-- ========== slide_decks ==========
-- Bir eğitim slayt paketi (deck). Kişisel, organizasyon geneli veya sistem şablonu.
create table if not exists public.slide_decks (
  ...
  title text not null,
  description text,
  category text,                       -- 'yangin' | 'kkd' | 'yuksekte_calisma' | 'elektrik' | 'kimyasal' | 'genel' vb.
  cover_image_url text,                -- kapak görseli
  theme text default 'modern',         -- 'modern' | 'classic' | 'dark' | 'corporate'
  language text default 'tr',

  -- Görünürlük
  visibility text not null default 'private' check (visibility in ('private','organization','public_template')),
  is_template boolean not null default false,    -- true = şablon olarak kullanılabilir
  is_system_template boolean not null default false, -- platform tarafından hazırlanmış hazır şablon
...
-- ========== slides ==========
-- Bir deck içindeki tek slayt. İçerik JSONB olarak yapılandırılmış bloklar.
create table if not exists public.slides (
  ...
  -- İçerik blokları — esnek yapı
  -- Şema: { title?:string, subtitle?:string, body?:string, bullets?:string[],
  --         image_url?:string, video_url?:string, caption?:string,
  --         background_color?:string, text_color?:string, notes?:string }
  content jsonb not null default '{}'::jsonb,

  -- Arka plan / tema özelleştirmesi
  background_color text,
  background_image_url text,

  -- Notlar (sunum sırasında gösterilmez)
  speaker_notes text,
...
-- ========== slide_deck_revisions (opsiyonel versiyon geçmişi) ==========
create table if not exists public.slide_deck_revisions (
  id uuid primary key default gen_random_uuid(),
  deck_id uuid not null references public.slide_decks(id) on delete cascade,
  revision_number integer not null,
  snapshot jsonb not null, -- tüm deck + slaytlar
...
-- ========== question_bank ==========
-- Kategorilere göre tekrar kullanılabilir soru havuzu.
create table if not exists public.question_bank (
  ...
  correct_answer text, -- short_answer / true_false için düz cevap
  explanation text,

  category text,     -- 'yangin','kkd','genel' vb.
  ...
  correct_rate numeric(5,2), -- 0-100
...
-- ========== slide_media_assets ==========
-- Slaytlarda kullanılan görsel/video gibi medyalar için referans tablosu
...
create trigger trg_question_bank_updated_at before update on public.question_bank
  for each row execute function public.update_updated_at_column();
-- ============================================================
-- slide_count otomatik güncelleme
-- ============================================================
...
-- slides policies (inherit from deck)
