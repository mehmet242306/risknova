-- ================================================================
-- Migration 005: Incidents, DÖF, Ishikawa tables
-- İş Kazası, Ramak Kala, Meslek Hastalığı kayıt ve takip sistemi
-- ================================================================
...
create table if not exists incidents (
  ...
  incident_environment text,
  shift_start_time time,
  shift_end_time time,
  work_start_time time,
  general_activity text,
  ...
  -- Kaza bilgileri
  accident_cause_description text,
  accident_cause text,
  accident_cause_tool text,
  accident_city text,
  accident_district text,

  -- Meslek hastalığı (occupational_disease tipinde)
  disease_work_environment text,
  disease_detection_method text,
  disease_agent text,
  disease_agent_duration text,
  disability_level text,
  disease_diagnosis text,
  disease_diagnosis_date date,

  -- AI ve analiz
  ai_summary jsonb,
  dof_required boolean default false,
  ishikawa_required boolean default false,
...
-- ================================================================
-- incident_witnesses - Şahit kayıtları
-- ================================================================
...
-- ================================================================
-- incident_dof - DÖF (Düzeltici ve Önleyici Faaliyet)
-- ================================================================
...
drop trigger if exists set_dof_updated_at on incident_dof;
create trigger set_dof_updated_at
  before update on incident_dof
  for each row execute function set_current_timestamp_updated_at();
-- ================================================================
-- incident_ishikawa - Balıkkılçığı Analizi
-- ================================================================
...
drop trigger if exists set_ishikawa_updated_at on incident_ishikawa;
create trigger set_ishikawa_updated_at
  before update on incident_ishikawa
  for each row execute function set_current_timestamp_updated_at();
-- ================================================================
-- RLS Policies
-- ================================================================
...
create policy "incidents_delete" on incidents for delete
  using (organization_id = current_organization_id());
-- witnesses policies (through incident ownership)
drop policy if exists "witnesses_select" on incident_witnesses;
drop policy if exists "witnesses_insert" on incident_witnesses;
drop policy if exists "witnesses_update" on incident_witnesses;
drop policy if exists "witnesses_delete" on incident_witnesses;
create policy "witnesses_select" on incident_witnesses for select
  using (exists (
    select 1 from incidents where incidents.id = incident_witnesses.incident_id
    and incidents.organization_id = current_organization_id()
  ));
create policy "witnesses_insert" on incident_witnesses for insert
  with check (exists (
    select 1 from incidents where incidents.id = incident_witnesses.incident_id
    and incidents.organization_id = current_organization_id()
  ));
create policy "witnesses_update" on incident_witnesses for update
  using (exists (
    select 1 from incidents where incidents.id = incident_witnesses.incident_id
    and incidents.organization_id = current_organization_id()
  ));
create policy "witnesses_delete" on incident_witnesses for delete
  using (exists (
    select 1 from incidents where incidents.id = incident_witnesses.incident_id
    and incidents.organization_id = current_organization_id()
  ));
-- dof policies
drop policy if exists "dof_select" on incident_dof;
drop policy if exists "dof_insert" on incident_dof;
drop policy if exists "dof_update" on incident_dof;
drop policy if exists "dof_delete" on incident_dof;
create policy "dof_select" on incident_dof for select
  using (organization_id = current_organization_id());
create policy "dof_insert" on incident_dof for insert
  with check (organization_id = current_organization_id());
create policy "dof_update" on incident_dof for update
  using (organization_id = current_organization_id());
create policy "dof_delete" on incident_dof for delete
  using (organization_id = current_organization_id());
-- ishikawa policies
drop policy if exists "ishikawa_select" on incident_ishikawa;
drop policy if exists "ishikawa_insert" on incident_ishikawa;
drop policy if exists "ishikawa_update" on incident_ishikawa;
drop policy if exists "ishikawa_delete" on incident_ishikawa;
create policy "ishikawa_select" on incident_ishikawa for select
  using (organization_id = current_organization_id());
create policy "ishikawa_insert" on incident_ishikawa for insert
  with check (organization_id = current_organization_id());
create policy "ishikawa_update" on incident_ishikawa for update
  using (organization_id = current_organization_id());
create policy "ishikawa_delete" on incident_ishikawa for delete
  using (organization_id = current_organization_id());
