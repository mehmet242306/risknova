...
-- Mevcut risk_assessments tablosuna ek kolonlar + yeni tablolar

begin;
-- ══════════════════════════════════════════════════════════════
-- risk_assessments tablosuna ek kolonlar
-- ══════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════
-- Risk analizi satırları (her satır bir risk konusu grubu)
-- ══════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════
-- Görseller (Supabase Storage referansları)
-- ══════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════
-- Tespitler (findings) — görsele bağlı
-- ══════════════════════════════════════════════════════════════
...
create table if not exists public.risk_assessment_findings (
  ...
  -- Skorlama verileri (JSONB — metoda göre değişir)
...
create index if not exists idx_risk_assessment_findings_row_id
  on public.risk_assessment_findings (row_id);
create index if not exists idx_risk_assessment_findings_image_id
  on public.risk_assessment_findings (image_id);
create index if not exists idx_risk_assessment_findings_assessment_id
  on public.risk_assessment_findings (assessment_id);
create index if not exists idx_risk_assessment_findings_severity
  on public.risk_assessment_findings (severity);
...
create policy "Users can view their org rows" on public.risk_assessment_rows
  for select using (organization_id = public.current_organization_id());
create policy "Users can insert their org rows" on public.risk_assessment_rows
  for insert with check (organization_id = public.current_organization_id());
create policy "Users can update their org rows" on public.risk_assessment_rows
  for update using (organization_id = public.current_organization_id());
create policy "Users can delete their org rows" on public.risk_assessment_rows
...
create policy "Users can view their org images" on public.risk_assessment_images
  for select using (organization_id = public.current_organization_id());
create policy "Users can insert their org images" on public.risk_assessment_images
  for insert with check (organization_id = public.current_organization_id());
create policy "Users can update their org images" on public.risk_assessment_images
  for update using (organization_id = public.current_organization_id());
create policy "Users can delete their org images" on public.risk_assessment_images
  for delete using (organization_id = public.current_organization_id());
-- Findings
...
create policy "Users can view their org findings" on public.risk_assessment_findings
  for select using (organization_id = public.current_organization_id());
create policy "Users can insert their org findings" on public.risk_assessment_findings
  for insert with check (organization_id = public.current_organization_id());
create policy "Users can update their org findings" on public.risk_assessment_findings
  for update using (organization_id = public.current_organization_id());
create policy "Users can delete their org findings" on public.risk_assessment_findings
  for delete using (organization_id = public.current_organization_id());
-- ══════════════════════════════════════════════════════════════
-- Storage bucket for risk analysis images
-- ══════════════════════════════════════════════════════════════
...
create policy "Users can upload risk images" on storage.objects
  for insert with check (bucket_id = 'risk-images' and auth.role() = 'authenticated');
create policy "Users can view risk images" on storage.objects
  for select using (bucket_id = 'risk-images' and auth.role() = 'authenticated');
create policy "Users can delete risk images" on storage.objects
  for delete using (bucket_id = 'risk-images' and auth.role() = 'authenticated');
comment on table public.risk_assessment_rows is 'Risk analysis row groups — each row represents a risk topic area with images.';
comment on table public.risk_assessment_images is 'Images uploaded for visual risk analysis, stored in Supabase Storage.';
comment on table public.risk_assessment_findings is 'Individual risk findings detected from images, with scoring and annotations.';
commit;
